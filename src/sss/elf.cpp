...
uint8_t *
sss_create(uint8_t *rodata, uint64_t rodata_size, uint8_t *text, uint64_t text_size, uint64_t entry) {
    uint64_t result_size = sizeof(Sss_Header) + rodata_size + text_size;
    uint8_t *result = (uint8_t *)urq_alloc(result_size);

    Sss_Header *header = (Sss_Header *)result;
    ...
    header->rodata_offset = sizeof(Sss_Header);
    header->rodata_size   = rodata_size;
    header->text_offset   = header->rodata_offset + header->rodata_size;
    header->text_size     = text_size;

    uint8_t *rodata_ptr = result + header->rodata_offset;
    uint8_t *text_ptr   = result + header->text_offset;

    memcpy(rodata_ptr, rodata, rodata_size);
...
bool
sss_valid(uint8_t *bin) {
    if ( !bin ) {
        return false;
    }

    if ( bin[EI_MAG0] != ELF_MAGIC_NUM1 ||
         bin[EI_MAG1] != ELF_MAGIC_NUM2 ||
         bin[EI_MAG2] != ELF_MAGIC_NUM3 ||
         bin[EI_MAG3] != ELF_MAGIC_NUM4 )
    {
        return false;
    }
...
uint8_t *
sss_text_section(uint8_t *bin) {
    if ( !sss_valid(bin) ) {
        report_error(&loc_none, "ungültige sss datei übergeben");
    }

    Sss_Header *header = (Sss_Header *)bin;
...
uint64_t
sss_text_num_entries(uint8_t *bin) {
    if ( !sss_valid(bin) ) {
        report_error(&loc_none, "ungültige sss datei übergeben");
    }
